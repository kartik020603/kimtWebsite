...
export default function Courses() {
    const courses = [
        ...
        { name: "Artificial Intelligence", duration: "18 Months", icon: Cpu, color: "bg-indigo-100 text-indigo-600", desc: "Dive deep into Machine Learning, Neural Networks and Cognitive Computing." },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 animate-in fade-in duration-1000">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {courses.map((course, i) => (
                    <div key={i} className="group p-8 rounded-3xl bg-white border border-gray-100 hover:border-indigo-200 hover:shadow-2xl hover:shadow-indigo-50 transition-all duration-300">
                        ...
                        <p className="text-sm font-semibold text-indigo-500 mb-4">{course.duration}</p>
                        <p className="text-gray-600 mb-8 leading-relaxed">{course.desc}</p>
                        <Link href="/contact" className="flex items-center gap-2 text-gray-900 font-bold hover:text-indigo-600 transition-colors">
                            Enroll Now
