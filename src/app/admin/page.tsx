...
export default async function AdminDashboard() {
    ...
    const stats = [
        ...
        { label: "Total Students", value: totalStudentRecords.length.toString(), icon: CreditCard, color: "text-blue-600", bg: "bg-blue-50" },
    ];
    ...
    return (
        <div className="space-y-10 pb-10">
                ...
                <div className="flex gap-4 w-full md:w-auto">
                    <form action="/admin/students" method="GET" className="relative w-full md:w-64">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-white border border-gray-100 rounded-xl font-bold text-gray-700 shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all"
                        />
                    </form>
                    <Link href="/admin/reports" className="flex items-center gap-2 bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold hover:bg-blue-700 transition-all shadow-md">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {stats.map((stat, i) => (
                    <div key={i} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm transition-all hover:shadow-blue-100/30">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <Link href="/admin/payments" className="text-blue-600 text-sm font-bold hover:underline">View All</Link>
                    </div>
                    <div className="space-y-6">
                        {recentPayments.map((p) => (
                            <div key={p.id} className="flex items-center justify-between group">
                                <div className="flex items-center gap-3">
                                    <div className="h-10 w-10 bg-gray-50 text-gray-400 rounded-xl flex items-center justify-center font-bold text-xs group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors uppercase">
                                        {p.student.user.name ? p.student.user.name[0] : p.student.user.username[0]}
                                    </div>
                                    <div>
                                        <p className="text-sm font-bold text-gray-900 group-hover:text-blue-600 transition-colors uppercase">{p.student.user.name || p.student.user.username}</p>
                                ...
                                <div className="text-right">
                                    ...
                                    <ArrowRight className="h-3 w-3 text-gray-300 ml-auto group-hover:text-blue-400 transition-colors" />
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="space-y-8">
                        <div className="space-y-4">
                            ...
                            <div className="flex justify-between text-[10px] font-black uppercase tracking-tight">
                                <span className="text-blue-600">₹{(globalTotalFees - globalTotalDue).toLocaleString()} Collected</span>
                                <span className="text-rose-500">₹{globalTotalDue.toLocaleString()} Pending</span>
                            </div>
                        </div>

                        <div className="pt-6 border-t border-gray-50">
                            <Link href="/admin/reports" className="block w-full py-4 text-center text-xs font-black text-blue-600 bg-blue-50 rounded-2xl hover:bg-blue-100 transition-all uppercase tracking-widest">
                                Analyze Performance
                            </Link>
                        </div>
                    </div>
